...
Namespace Part
	''' <summary>
	''' Creates a new part and adds a new chamfer using AddUnequalSetback().
	''' </summary>
	Friend Class CreateChamferByAddUnequalSetback
		Private Shared Sub RunSample(ByVal breakOnStart As Boolean)
			...
			Try
				...
				faces = DirectCast(extrudedProtrusion.Faces(SolidEdgeGeometry.FeatureTopologyQueryTypeConstants.igQueryAll), SolidEdgeGeometry.Faces)

				chamfers = model.Chamfers

				Dim face As SolidEdgeGeometry.Face = DirectCast(faces.Item(1), SolidEdgeGeometry.Face)
				Dim setbackDistance1 As Double = 0.009
				Dim setbackDistance2 As Double = 0.001

				edges = DirectCast(face.Edges, SolidEdgeGeometry.Edges)
				edgeList.Add(edges.Item(1))

				chamfer = chamfers.AddUnequalSetback(ReferenceFace:= face, NumberOfEdgeSets:= edgeList.Count, EdgeSetArray:= edgeList.ToArray(), SetbackDistance1:= setbackDistance1, SetbackDistance2:= setbackDistance2)
